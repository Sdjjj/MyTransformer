...
n_heads = 8 # 多头注意力中的head数目
batch_size = 3
n_decoder_layers = 6 # Decoder的层数
n_encoder_layers = 6 # Encoder的层数

device = 'cuda' if torch.cuda.is_available() else 'cpu'

# 定义正弦位置编码表函数
def get_sin_encode_table(n_position, embedding_dim):
    #-------------------------维度信息--------------------------------
    # n_position: 输入序列的最大长度(即一个句子中的最多单词数)
    ...
    for pos_i in range(n_position):
        for hid_j in range(embedding_dim):
            angle = pos_i / np.power(1000, 2 * (hid_j // 2) / embedding_dim)
            sinusoid_table[pos_i, hid_j] = angle
    
    # 计算正弦和余弦值
    sinusoid_table[:, 0::2] = np.sin(sinusoid_table[:, 0::2]) # 偶数
    sinusoid_table[:, 1::2] = np.cos(sinusoid_table[:, 1::2]) # 奇数

    return torch.FloatTensor(sinusoid_table)

# 生成填充注意力掩码的函数(用于多头注意力中)
def get_attn_pad_mask(seq_q, seq_k):
    ...
    # 将pad_attn_mask扩展为与seq_q 和 seq_k 相同形状的张量
...
def get_attn_subsequent_mask(seq):
    ...
    # 将numpy数组转换为pytorch张量，并将数据类型设置为byte
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        # 将注意力掩码复制到多头中
        # 这个操作的目的是将原始的 attn_mask 复制到每个注意力头（head）上，以便每个头都能够独立地使用相同的掩码进行自注意力计算
        attn_mask = attn_mask.unsqueeze(1).repeat(1, n_heads, 1, 1)
        #-------------------------维度信息-------------------------------- 
        # attn_mask [batch_size, n_heads, len_q, len_k]
        #-------------------------维度信息-------------------------------- 
        
        # 使用缩放点积注意力计算context和权重
...
class Encoder(nn.Module):
    def __init__(self, corpus):
        ...
        self.pos_emb = nn.Embedding.from_pretrained(get_sin_encode_table(corpus.src_len+1, d_embedding), freeze=True) # 位置嵌入层
        self.layers = nn.ModuleList(EncoderLayer() for _ in range(n_encoder_layers))# 编码器层数
...
class Decoder(nn.Module):
    def __init__(self, corpus):
        ...
        self.pos_emb = nn.Embedding.from_pretrained(get_sin_encode_table(corpus.tgt_len+1, d_embedding), freeze=True) # 位置嵌入层        
        self.layers = nn.ModuleList([DecoderLayer() for _ in range(n_decoder_layers)]) # 叠加多层
